...
@Component({
      ...
      <div class="config-card">
        ...
        <div class="card-body">
          @if (isLoading()) {
            ...
            <div class="stats-grid">
              ...
              <div class="stat-item">
                <span class="stat-value">
                  {{ formatBytes(config()?.stats?.total_size_bytes || 0) }}
                </span>
            ...
            <!-- Seletor de Modelo de Embedding -->
            <div class="embedding-selector">
              <h3>Modelo de Embedding</h3>
              <p class="selector-desc">Escolha o modelo de acordo com o idioma do seu conteudo</p>
              <div class="model-options">
                @for (model of embeddingModels(); track model.short_name) {
                  <div
                    class="model-option"
                    [class.selected]="currentModel() === model.short_name"
                    [class.changing]="isChangingModel() && selectedModel() === model.short_name"
                    (click)="selectModel(model.short_name)"
                  >
                    <div class="model-header">
                      <span class="model-name">{{ model.display_name }}</span>
                      <span class="model-lang" [class.pt]="model.language === 'pt-br'">
                        {{ model.language === 'pt-br' ? 'PT-BR' : 'EN' }}
                      </span>
                    </div>
                    <div class="model-details">
                      <span class="model-dims">{{ model.dimensions }}d</span>
                      @if (currentModel() === model.short_name) {
                        <span class="model-current">Atual</span>
                      }
                    </div>
                    @if (isChangingModel() && selectedModel() === model.short_name) {
              ...
              @if (modelChangeMessage()) {
                <div
                  class="model-message"
                  [class.success]="!modelChangeError()"
                  [class.error]="modelChangeError()"
                >
                  {{ modelChangeMessage() }}
                </div>
              }
          ...
          <div class="actions-grid">
            <div class="action-item">
              <button
                ...
                (click)="reingestBackend()"
                [disabled]="isReingesting()"
              >
                {{
                  isReingesting() && currentAction() === 'backend'
                    ? 'Reingestando...'
                    : 'Reingestar Backend'
                }}
              </button>
            ...
            <div class="action-item">
              ...
              >
                {{
                  isReingesting() && currentAction() === 'sdk'
                    ? 'Reingestando...'
                    : 'Reingestar Claude SDK'
                }}
              </button>
            ...
            <div class="action-item">
              <button
                class="action-btn angular-btn"
                (click)="reingestAngular()"
                [disabled]="isReingesting()"
              >
                {{
                  isReingesting() && currentAction() === 'angular'
                    ? 'Reingestando...'
                    : 'Reingestar Angular Docs'
                }}
              </button>
            ...
            <div class="action-item">
              <button
                class="action-btn atlantyx-btn"
                (click)="reingestAtlantyx()"
                [disabled]="isReingesting()"
              >
                {{
                  isReingesting() && currentAction() === 'atlantyx'
                    ? 'Reingestando...'
                    : 'Reingestar Atlantyx'
                }}
              </button>
              <span class="action-desc">Documentos da Prova Atlantyx (PDFs, DOCXs, HTMLs)</span>
            </div>
          </div>
          @if (actionOutput()) {
            <div
              class="action-output"
              [class.success]="!actionError()"
          ...
          <div class="watcher-section">
            <div class="watcher-info">
              ...
              <div class="watcher-content">
                ...
                <p>
                  Monitora mudanças em arquivos Python e executa reingestão automaticamente
                  (cooldown: 5s).
                </p>
      ...
      <div class="config-card danger-zone">
        ...
        <div class="card-body">
          <p class="danger-description">
            Apagar o banco de dados RAG remove permanentemente todos os documentos indexados e seus
            embeddings. Esta ação não pode ser desfeita.
          </p>
          ...
          <div class="roadmap-list">
            <div class="roadmap-item">
              ...
              <div class="roadmap-content">
                ...
                <p>
                  Escolher entre BGE Small (rápido), Base (balanceado) ou Large (maior precisão).
                </p>
            ...
            <div class="roadmap-item">
              ...
              <div class="roadmap-content">
                ...
                <p>
                  Ajustar tamanho dos chunks, overlap e estratégia (fixo, sentença, parágrafo,
                  semântico).
                </p>
  ...
  styles: [
    `
      ...
      .action-btn.atlantyx-btn {
        background: #c87533;
        color: white;
      }
      .action-btn.atlantyx-btn:hover:not(:disabled) {
        background: #a85d20;
      }
      .action-btn:disabled {
        background: #9a9a9a;
      ...
      .doc-stat-item.backend .doc-stat-value {
        color: #3b82f6;
      }
      .doc-stat-item.sdk .doc-stat-value {
        color: #8b5cf6;
      }
      .doc-stat-item.angular .doc-stat-value {
        color: #dd0031;
      }
      .doc-stat-label {
        display: block;
...
export class ConfigPageComponent implements OnInit {
  ...
  currentAction = signal<'backend' | 'sdk' | 'angular' | 'atlantyx' | null>(null);
  ...
  // Embedding models
  embeddingModels = signal<EmbeddingModelOption[]>([]);
  currentModel = signal<string>('');
  selectedModel = signal<string>('');
  isChangingModel = signal(false);
  modelChangeMessage = signal<string | null>(null);
  modelChangeError = signal(false);
    ...
    try {
      ...
      for (const doc of docs) {
        ...
        if (nome.includes('angular') || nome.includes('angular-cli-mcp')) {
          ...
          nome.includes('sdk') ||
          nome.includes('claude-agent-sdk') ||
          nome.includes('claude_agent_sdk')
        ) {
          sdk++;
  ...
  async loadEmbeddingModels(): Promise<void> {
    try {
      const response = await fetch(`${this.configService.apiUrl}/rag/embedding-models`);
      const data = await response.json();
      this.embeddingModels.set(data.models || []);
      this.currentModel.set(data.current || '');
    } catch (err) {
      console.error('Erro ao carregar modelos de embedding:', err);
    }
  }

  async selectModel(shortName: string): Promise<void> {
    if (shortName === this.currentModel() || this.isChangingModel()) {
      return;
    }

    this.selectedModel.set(shortName);
    this.isChangingModel.set(true);
    this.modelChangeMessage.set(null);
    this.modelChangeError.set(false);

    try {
      const response = await fetch(
        `${this.configService.apiUrl}/rag/embedding-model?model=${shortName}`,
        {
          method: 'POST',
          headers: {
      ...
      if (response.ok && data.success) {
        this.currentModel.set(shortName);
        this.modelChangeMessage.set(data.message);
        this.modelChangeError.set(false);
        // Recarregar config para atualizar UI
        await this.loadConfig();
      } else {
        this.modelChangeError.set(true);
        this.modelChangeMessage.set(data.detail || 'Erro ao alterar modelo');
      }
    } catch (err: any) {
      this.modelChangeError.set(true);
      this.modelChangeMessage.set('Erro: ' + (err.message || 'Falha na conexao'));
    } finally {
  ...
  async reingestAtlantyx(): Promise<void> {
    this.currentAction.set('atlantyx');
    await this.doReingest(
      '/rag/reingest/atlantyx',
      '⏳ Processando documentos Atlantyx (PDFs, DOCXs, HTMLs)...'
    );
  }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          this.actionOutput.set(
            `✗ Erro ${response.status}: ${errorData.detail || response.statusText}`
          );
        }
        return;
      }
  ...
  private async doReingest(endpoint: string, progressMessage: string): Promise<void> {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        this.actionOutput.set(
          '✗ ERRO: Timeout após 5 minutos. O script pode ainda estar rodando no backend.'
        );
      } else {
