            ...
            <div class="empty-state">
              <div class="empty-illustration">
                ...
                <svg width="120" height="100" viewBox="0 0 120 100" fill="none">
                  <!-- Mao -->
                  <path d="M35 95 L35 55 Q35 50 40 50 L45 50 L45 35 Q45 30 50 30 Q55 30 55 35 L55 50 L60 50 L60 25 Q60 20 65 20 Q70 20 70 25 L70 50 L75 50 L75 30 Q75 25 80 25 Q85 25 85 30 L85 60 L85 95" stroke="#9a9a9a" stroke-width="2" fill="none"/>
                  <!-- Retangulo/Card -->
                  <rect x="25" y="10" width="30" height="40" rx="3" stroke="#9a9a9a" stroke-width="2" fill="#e5e4df"/>
                  <!-- Circulo -->
              ...
              <p>Se voce pode sonhar, voce pode construir. Transforme apps, jogos, templates e ferramentas de ideias em realidade.</p>
            ...
            <ul class="file-list">
              @for (file of files(); track file.name) {
                <li class="file-item">
                  ...
                  <div class="file-info">
                    <a [href]="getFileUrl(file)" class="file-name" target="_blank">{{ file.name }}</a>
...
export class ArtifactsPageComponent implements OnInit {
  ...
  inspirationItems = signal([
    { title: 'Dashboard Analytics', author: 'Claude', icon: '📊', gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
    { title: 'Landing Page', author: 'Claude', icon: '🚀', gradient: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)' },
    { title: 'Todo App', author: 'Claude', icon: '✅', gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
    { title: 'Quiz Game', author: 'Claude', icon: '🎮', gradient: 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)' },
    { title: 'Calculadora', author: 'Claude', icon: '🔢', gradient: 'linear-gradient(135deg, #fa709a 0%, #fee140 100%)' },
    { title: 'Pomodoro Timer', author: 'Claude', icon: '⏱️', gradient: 'linear-gradient(135deg, #a8edea 0%, #fed6e3 100%)' },
  ]);
  ...
  async loadFiles(): Promise<void> {
    ...
    try {
      const response = await fetch(`${apiUrl}/outputs`);
  ...
  getFileUrl(file: OutputFile): string {
    ...
    return `${apiUrl}/outputs/file/${filePath}`;
  }
