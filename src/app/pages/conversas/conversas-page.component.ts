            ...
            <div class="conversa-item" [class.selected]="isSelected(conversa.id)">
              @if (isSelectMode()) {
                <label class="checkbox-container">
                  <input
                    type="checkbox"
                    [checked]="isSelected(conversa.id)"
                    (change)="toggleSelect(conversa.id)"
                  />
              ...
              <div class="conversa-content" [routerLink]="['/chat', conversa.id]">
                <div class="conversa-header">
                  ...
                  <span class="conversa-titulo" [title]="conversa.titulo">
                    {{ conversa.titulo }}
                  </span>
                  @if (conversa.isFirst) {
...
export class ConversasPageComponent implements OnInit {
  ...
  conversas = computed(() => {
    ...
    return sortedSessions.map((session, index) => this.sessionToConversa(session, index === 0));
  });
  ...
  private sessionToConversa(session: Session, isFirst = false): Conversa {
    const isJsonl =
      session.file_name?.endsWith('.jsonl') || session.file?.includes('.jsonl') || false;
  ...
  filteredConversas() {
    ...
    return this.conversas().filter(c => c.titulo.toLowerCase().includes(query));
  }
