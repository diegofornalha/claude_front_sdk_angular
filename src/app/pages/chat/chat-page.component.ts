...
import {
  OutputsService,
  OutputFile,
} from '../../../../projects/claude-front-sdk/src/lib/services/outputs.service';
...
@Component({
  ...
  template: `
    ...
    @if (isOutputsFullPage()) {
          ...
          <div class="outputs-card">
            <div class="card-header">
              ...
              <span class="files-count">
                {{ outputs.files().length }} arquivo{{ outputs.files().length !== 1 ? 's' : '' }}
              </span>
            </div>
            <ul class="file-list">
              @if (outputs.isLoading()) {
                ...
                @for (file of outputs.files(); track file.name) {
                  <li class="file-item">
                    ...
                    <div class="file-info">
                      <a class="file-name" [href]="getOutputFileUrl(file)" target="_blank">
                        {{ file.name }}
                      </a>
                      <div class="file-meta">
                        {{ outputs.formatSize(file.size) }} -
                        {{ outputs.formatDate(file.modified) }}
                      </div>
                    </div>
                    <div class="file-actions">
                      <button class="action-btn" (click)="downloadFile(file)">Download</button>
                      <button class="action-btn delete" (click)="deleteOutputFile(file)">
                        Apagar
                      </button>
                    </div>
                  </li>
        ...
        <div class="anonymous-header">
          <div class="anonymous-title">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
              <path
                d="M12 2a9 9 0 0 0-9 9c0 3.03 1.53 5.82 4 7.47V22l3.5-2 3.5 2v-3.53c2.47-1.65 4-4.44 4-7.47a9 9 0 0 0-9-9zm-3 10c-.83 0-1.5-.67-1.5-1.5S8.17 9 9 9s1.5.67 1.5 1.5S9.83 12 9 12zm6 0c-.83 0-1.5-.67-1.5-1.5S14.17 9 15 9s1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"
              />
            </svg>
            <span>Chat anônimo</span>
          </div>
          <button
            class="close-anonymous-btn"
            (click)="exitAnonymousMode()"
            title="Sair do modo anônimo"
          >
            <svg
              width="20"
              height="20"
      ...
      <div
        class="chat-page"
        [class.panel-open]="isArtifactsPanelOpen() || isOutputsPanelOpen()"
        [class.anonymous]="isAnonymousMode()"
      >
          ...
          @if (isArtifactMode() && !hasMessages()) {
            <div class="artifact-mode">
              ...
              <div class="artifact-content">
                <h2>
                  Maos a obra! Escolha uma categoria de artefato ou comece a construir sua ideia do
                  zero.
                </h2>
        ...
        @if (!isArtifactsPanelOpen() && !isOutputsPanelOpen()) {
          <div class="floating-buttons">
            ...
            @if (currentSessionId() && hasOutputFiles()) {
              <button
                class="floating-btn outputs-btn"
                (click)="toggleOutputsPanel()"
            ...
            @if (hasArtifacts()) {
              <button
                class="floating-btn artifacts-btn"
            ...
            @if (!hasArtifacts() && !hasOutputFiles() && !isAnonymousMode()) {
              <button
                class="floating-btn anonymous-btn"
                (click)="enterAnonymousMode()"
                title="Ativar chat anônimo"
              >
...
export class ChatPageComponent implements OnInit, OnDestroy {
  ...
  artifactCategories = signal<ArtifactCategory[]>([
    ...
    },
    {
      ...
      title: 'Jogos',
      iconSvg:
        '<rect x="2" y="6" width="20" height="12" rx="2"/><line x1="6" y1="12" x2="6" y2="12"/><line x1="18" y1="12" x2="18" y2="12"/>',
  ...
  constructor() {
    ...
    effect(() => {
      ...
      console.log('[ChatPage] Effect:', {
        sessionId,
        isNewChat: this.isNewChat,
        hasMessages,
        isAnon: this.isAnonymousMode(),
      });
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(async params => {
      ...
      const isValidSessionId =
        sessionId && (uuidRegex.test(sessionId) || customIdRegex.test(sessionId));
      if (isValidSessionId) {
  ...
  getOutputFileUrl(file: OutputFile): string {
    const sessionId = this.currentSessionId();
    if (sessionId) {
      return `${this.config.apiUrl}/artifacts/${sessionId}/${file.name}`;
    }
